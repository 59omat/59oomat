...
/**
 * رابط الغلاف العام (Supabase Storage) إن وُجد، وإلا undefined.
 * لا يُخزَّن غلاف بلا صور.
 */
  ...
  handler: async (ctx) => {
    ...
    return await Promise.all(
      sorted.map(async (d) => ({
        ...
        images: d.images,
        imageUrl: d.imageUrl ?? null,
        // رابط Supabase العام أولًا، وإلا أول صورة في مخزن Convex.
        thumbnail:
          d.imageUrl ??
          (d.images.length ? await ctx.storage.getUrl(d.images[0]) : null),
      })),
    );
  },
});
...
export const create = mutation({
  args: {
    ...
    // رابط عام من Supabase Storage (اختياري، وnull يعني لا غلاف).
  ...
  handler: async (ctx, args) => {
    ...
    return await ctx.db.insert("discounts", {
      ...
      images: args.images,
      imageUrl: resolveCoverUrl(args.imageUrl, args.images),
