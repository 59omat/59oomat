...
/** رابط Supabase العام (إن وُجد) يُعرض أولًا، ثم صور مخزن Convex. */
function orderImages(imageUrl: string | null, imageUrls: string[]): string[] {
  return [...(imageUrl ? [imageUrl] : []), ...imageUrls];
}
...
export function fromNearby(row: NearbyRow, distance: number): DiscountView {
  return {
    ...
    endDate: row.endDate,
    images: orderImages(row.imageUrl, row.imageUrls),
    views: row.views,
    distanceKm: distance,
    store: {
      id: row.store._id,
