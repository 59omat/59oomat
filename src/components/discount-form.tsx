...
import { uploadOfferImages } from "@/lib/upload";
...
export type DiscountFormValues = {
  ...
  endDate: number;
  images: Id<"_storage">[];
  /** رابط عام (Supabase Storage) لصورة الغلاف، إن توفّرت. */
...
export function DiscountForm({
  ...
  async function handleImages(files: FileList | null) {
    ...
    setUploading(true);
    try {
      const { ids, coverUrl: uploadedCover } = await uploadOfferImages(
        picked,
        generateUploadUrl,
      );
      if (uploadedCover) setCoverUrl(uploadedCover);
  ...
  function handleRemoveImage(index: number) {
    ...
    if (nextImages.length === 0) setCoverUrl(null);
  }
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      await onSubmit({
        ...
        images,
        imageUrl: images.length ? coverUrl : null,
      });
    } catch (error) {
      ...
      <div className="space-y-1.5">
        ...
        <p className="text-[10px] text-muted-foreground">
          {isSupabaseConfigured
            ? "الصور تُرفع إلى مخزن التطبيق، وتُرفع صورة الغلاف إلى Supabase Storage لرابط عام سريع (CDN)."
